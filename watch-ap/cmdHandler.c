...
static u8 directionControlValue = 0;
static u8 speedControlValue = 0;
...
void usb_decode(void)
{
  ...
  switch (usb_buffer[PACKET_BYTE_CMD])
  {
    ...
    case BM_GET_SIMPLICITIDATA:
                              receivedData = getFlag(simpliciti_flag, SIMPLICITI_TRIGGER_RECEIVED_DATA);
      
                              if (receivedData)
                              {
                                // Assemble IN packet ControlValue and ControlMode.
                                if (simpliciti_data[0] == 1)
                                    directionControlValue = simpliciti_data[1];
                                else if (simpliciti_data[0] == 2)
                                    speedControlValue = simpliciti_data[1];
                              ...
                              usb_buffer[PACKET_BYTE_FIRST_DATA + 3] = speedControlValue;
                              usb_buffer[PACKET_BYTE_FIRST_DATA + 2] = directionControlValue;
                              usb_buffer[PACKET_BYTE_FIRST_DATA + 1] = receivedData;
                              break;
